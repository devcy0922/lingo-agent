...
- 용어 정확성은 항목별 4점 이상을 강제합니다. 의미 정확성·자연스러움·UI 적합성은 3점 이상이면서 항목 평균이 3.5점 이상이어야 통과합니다.
- 의미 정확성 3점은 개선 권고로 기록하되 단독 차단 사유로 사용하지 않고, 2점 이하이거나 `critical_errors`가 동반될 때 차단합니다.
